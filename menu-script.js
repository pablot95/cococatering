...
// Actualizar cantidad de box salados y agregar directamente al carrito
function updateBoxQty(button, change, boxId) {
    const controls = button.closest('.product-controls');
    const qtyDisplay = controls.querySelector('.qty-display');
    const container = button.closest('.box-container');
    ...
    if (newQty > currentQty) {
        // Agregar al carrito
        const product = {
            id: container.dataset.id || boxId,
            name: container.dataset.name,
            price: parseInt(container.dataset.price),
            image: container.dataset.image || 'images/salados.jpg',
            quantity: 1
        };
        ...
        // Remover del carrito
        const productId = container.dataset.id || boxId;
...
// Actualizar cantidad de box dulces y agregar directamente al carrito
function updateBoxDulceQty(button, change, boxId) {
    const controls = button.closest('.product-controls');
    const qtyDisplay = controls.querySelector('.qty-display');
    const boxOption = button.closest('.box-option');
    ...
    if (newQty > currentQty) {
        // Agregar al carrito
        const product = {
            id: boxOption.dataset.id || boxId,
            name: boxOption.dataset.name,
            price: parseInt(boxOption.dataset.price),
            image: boxOption.dataset.image || 'productos/BOX-PATTISERIE.jpg',
            quantity: 1
        };
        ...
        // Remover del carrito
        const productId = boxOption.dataset.id || boxId;
        if (typeof removeFromCart === 'function') {
            removeFromCart(productId);
        }
    }
